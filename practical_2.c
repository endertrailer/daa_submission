...
int findMinIndex(float cost[], int n, int path[], int pathCount) {
  float min = INFINITY;
  int minIndex = -1;
  for (int i = 0; i < n; i++) {
    if (cost[i] < min && !isAlreadyVisited(path, pathCount, i)) {
      min = cost[i];
      minIndex = i;
    }
  }
...
int main() {
  ...
  while (pathCount < n) {
    int nextNode = findMinIndex(cost[currentNode], n, finalPath, pathCount);
    if (nextNode == -1)
      break;
